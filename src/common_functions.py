...
#         W =theano.shared(name='W', value=W.astype(theano.config.floatX), borrow=True)
        ...
        W = theano.shared(numpy.asarray(
            rng.uniform(low=-0.01, high=0.01, size=(fan_in,fan_out)),
...
#     def __init__(self, X, Mask, hidden_dim, U, W, b):
    def __init__(self, X, Mask, hidden_size, tparams ):
        ...
        rval, updates = theano.scan(_step,
                                    ...
                                    n_steps=nsteps)
        self.output_tensor = rval[0]#.dimshuffle(1,2,0) # "_step" function returns two variables "h" and "c", only "h" is useful, "c" will be discarded. (nsamples, batch, hidden_size)
        self.output_sent_rep=self.output_tensor[-1,:,:]#[:,:,-1] # choose the last hidden state as sentence representation
...
def cosine_matrix_matrix(M1, M2):
    #M1 (batch, hidden), M2 (classes, hidden), output similarity (batch, classes)
    dots=M1.dot(M2.T) #(batch, classies)
    norm1= T.sqrt(1e-8+T.sum(M1**2, axis=1)) #(batch)
    norm2= T.sqrt(1e-8+T.sum(M2**2, axis=1)) #(classies)
    norm_dot=1e-8+norm1.dimshuffle(0,'x').dot(norm2.dimshuffle('x', 0))
    return dots/norm_dot #(batch, classies)

def distance_matrix_matrix(M1, M2):
    #M1 (batch, hidden), M2 (classes, hidden), output similarity (batch, classes)
    M1_rep = T.repeat(M1.dimshuffle('x',0,1), M2.shape[0], axis=0).reshape((M2.shape[0]*M1.shape[0], M1.shape[1])) #(classes*batch, hidden)
    M2_rep = T.repeat(M2, M1.shape[0], axis=1).reshape((M2.shape[0]*M1.shape[0], M2.shape[1]))
    
    distance=T.sum(T.sqr(M1_rep - M2_rep), axis=1).reshape((M1.shape[0], M2.shape[0])) #(batch, classes)
    raw = 1.0/(1.0+distance)
    
    return raw/T.sum(raw, axis=1).dimshuffle(0,'x') #(batch, classies)
