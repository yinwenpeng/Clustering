...
def remove_noisewords(wordlist, vocab):
    new_list=[]
    for word in wordlist:
        if word in vocab:
            new_list.append(word)
    return new_list
def load_20news(maxDocLen, vocab):
    rootPath='/mounts/data/proj/wenpeng/Dataset/20news-bydate-'
    folders=['alt.atheism',
             'comp.graphics',
    ...
    for class_id, folder in enumerate(folders):
        for Split in Splits: 
            folder_path=rootPath+Split+folder+'/'
...
#                 print 'loading', file_path, '...'
                ...
                for line in readfile:
                    if 'Lines:'  in line:
                        valid=True
                        continue
                    if valid and line.strip()!='':
                        doc_str+=line.strip().lower()
                doc_wordlist=replacePunctuationsInStrBySpace(doc_str).split()
                doc_wordlist = remove_noisewords(doc_wordlist, vocab)
                doc_ids = strs2ids(doc_wordlist, word2id)

                #pad
                valid_size=len(doc_ids)
                pad_size=maxDocLen-valid_size
                if pad_size > 0:
                    one_doc=[0]*pad_size+doc_ids
                    one_mask=[0.0]*pad_size+[1.0]*valid_size
                else:
                    one_doc=doc_ids[:maxDocLen]
                    one_mask=[1.0]*maxDocLen
                docs.append(one_doc)
                masks.append(one_mask)
                labels.append(class_id)
    print 'load 20news over, totally', len(docs), 'docs'
    return     word2id,docs,masks, labels
...
if __name__ == '__main__':
    load_relationPath_and_labels(20)
